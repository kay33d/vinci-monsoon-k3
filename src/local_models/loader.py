...
HYBRID ARCHITECTURE (three lanes, cheapest first):
  1. Stage-1 category detection is a pure keyword heuristic — zero tokens,
     zero latency, no weights. The 0.5B-GGUF *classifier* experiment was
     measurably inaccurate, so classification NEVER uses the model.
  2. A bundled quantized Qwen GGUF (llama-cpp-python, CPU) answers the
     ...
     Fireworks tokens. The model is LAZY-LOADED on first use — startup on
     the 2-vCPU grading box counts against the 10-minute wall clock — and
     generation is serialized behind a lock (llama.cpp instances are not
     thread-safe) while remote tasks proceed in parallel around it.
